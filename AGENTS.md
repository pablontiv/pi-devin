...
- Never hardcode a fallback Desktop version. Resolve `windsurfVersion` from an installed Devin Desktop, a validated six-hour cache, or the fixed official update manifest; `DEVIN_CLIENT_VERSION` is an explicit emergency/test override. Fail closed when no verified version is available.
